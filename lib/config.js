 ...
 *
 * Recognized properties include `witnesses` (witness service URLs), `logs`
 * and `secrets` (output directories), and `recognizedWitnesses` — an array
 * of `{id, validFrom, validUntil}` entries identifying the witnesses whose
 * proofs are accepted when resolving a did:cel from a storage service.
 *
...
export function loadConfig({configPath = DEFAULT_CONFIG_PATH} = {}) {
  ...
  Object.assign(config, {
    ...
    recognizedWitnesses: raw.recognizedWitnesses ?? []
  });
}
